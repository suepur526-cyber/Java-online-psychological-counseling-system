...
async function login(page, role, account, password) {
  ...
  await expect(page.getByTestId('app-sidebar')).toContainText(role === '普通用户' ? '服务预约' : role === '心理老师' ? '服务预约审核' : '公告管理')
}
...
test('teacher can review appointment and reply consultation', async ({ page }) => {
  ...
  await expect(page.getByRole('row', { name: /端到端测试预约/ }).first()).toBeVisible()
  await page.getByText('通过').last().click()
  await page.getByRole('button', { name: 'OK' }).click()
  await expect(page.getByText('审核完成')).toBeVisible()

  await openMenu(page, '在线咨询管理')
  await expect(page.getByRole('row', { name: /端到端测试咨询内容/ }).first()).toBeVisible()
  await page.getByRole('row', { name: /端到端测试咨询内容/ }).first().getByRole('button', { name: '回复' }).click()
  await page.getByRole('button', { name: 'OK' }).click()
