...
test.describe.serial('full functional QA', () => {
  ...
  test('student menus, lists, dialogs and consultation flow are usable', async ({ page }) => {
    ...
    await openMenu(page, '服务预约')
    const chenService = page.locator('.service-card').filter({ hasText: '陈老师' })
    await expect(chenService).toContainText('情绪压力')
    await chenService.getByRole('button', { name: '服务预约' }).click()
    await expectDialogInsideViewport(page, '服务预约')
    ...
    await chenService.getByRole('button', { name: '留言咨询' }).click()
    await expectDialogInsideViewport(page, '用户留言咨询')
    await page.getByLabel('咨询内容').fill('全功能验收咨询内容')
    await page.getByRole('button', { name: '提交' }).click()
    await expect(page.getByText('咨询提交成功')).toBeVisible()

    await openMenu(page, '在线心理测试')
    ...
    await openMenu(page, '服务预约查询')
    await expect(page.getByRole('row', { name: /全功能验收预约/ }).first()).toBeVisible()
    await openMenu(page, '咨询回复查询')
    await expect(page.getByRole('row', { name: /全功能验收咨询内容/ }).first()).toBeVisible()
    await logout(page)
  })

  test('teacher menus, review dialogs, replies and test maintenance are usable', async ({ page }) => {
    const testName = `全功能验收测试${Date.now()}`
    await login(page, accounts.teacher)
    await expect(page.getByText('今日咨询工作台')).toBeVisible()

    await openMenu(page, '服务预约审核')
    await expect(page.getByRole('row', { name: /全功能验收预约/ }).first()).toBeVisible()
    await page.getByRole('row', { name: /全功能验收预约/ }).first().getByRole('button', { name: '通过' }).click()
    await expectDialogInsideViewport(page, '服务预约审核')
    await page.getByRole('button', { name: 'OK' }).click()
    await expect(page.getByText('审核完成')).toBeVisible()

    await openMenu(page, '在线咨询管理')
    await expect(page.getByRole('row', { name: /全功能验收咨询内容/ }).first()).toBeVisible()
    await page.getByRole('row', { name: /全功能验收咨询内容/ }).first().getByRole('button', { name: '回复' }).click()
    await expectDialogInsideViewport(page, '在线咨询管理')
    await page.getByRole('button', { name: 'OK' }).click()
    await expect(page.getByText('回复成功')).toBeVisible()

    await openMenu(page, '心理测试管理')
    await page.getByRole('button', { name: '新增测试' }).click()
    await expectDialogInsideViewport(page, '测试维护')
    await page.getByLabel('测试名称').fill(testName)
    ...
    await page.getByRole('row', { name: new RegExp(testName) }).getByRole('button', { name: '题目' }).click()
  ...
  test('admin menus, moderation, system settings and management dialogs are usable', async ({ page }) => {
    const announcementTitle = `全功能验收公告${Date.now()}`
    ...
    await expect(page.locator('.el-message', { hasText: '用户状态已更新' }).last()).toBeVisible()
    await openMenu(page, '心理老师信息')
    await expect(page.locator('.topbar h2')).toHaveText('心理老师信息')
    ...
    await expect(page.getByRole('row', { name: new RegExp(announcementTitle) })).toBeVisible()
    ...
    await openMenu(page, '系统管理模块')
    await expect(page.locator('.topbar h2')).toHaveText('系统管理模块')
  ...
  test('secondary buttons and negative branches are clickable and safe', async ({ page }) => {
    const edgeAppointmentNote = `边缘按钮驳回预约${Date.now()}`
    await page.goto('/login')
    ...
    await expect(page.getByRole('dialog', { name: '服务预约' })).toBeHidden()

    await chenService.getByRole('button', { name: '留言咨询' }).click()
    await expectDialogInsideViewport(page, '用户留言咨询')
    await page.getByRole('button', { name: '取消' }).click()
    await expect(page.getByRole('dialog', { name: '用户留言咨询' })).toBeHidden()

    await chenService.getByRole('button', { name: '服务预约' }).click()
    await page.getByLabel('预约时间').fill('2026-06-03 09:00:00')
    await page.getByLabel('预约备注').fill(edgeAppointmentNote)
    await page.getByRole('button', { name: '提交' }).click()
    await expect(page.getByText('预约提交成功')).toBeVisible()
    ...
    await login(page, accounts.teacher)
    await openMenu(page, '服务预约审核')
    await page.getByRole('row', { name: new RegExp(edgeAppointmentNote) }).getByRole('button', { name: '驳回' }).click()
    await expectDialogInsideViewport(page, '服务预约审核')
    await page.getByRole('button', { name: 'OK' }).click()
    await expect(page.getByText('审核完成')).toBeVisible()
    ...
    await page.getByRole('button', { name: '保存' }).click()
    await expect(page.getByText('公告已保存')).toBeVisible()
    await page.getByRole('row', { name: /全功能验收公告/ }).first().getByRole('button', { name: '删除' }).click()
